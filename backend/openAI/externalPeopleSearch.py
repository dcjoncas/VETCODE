    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "user", "content": f"From the following job description, extract the required skills as a comma separated list. Add no additional text or commentary: {jobDescription}"}
            ],
        ...
        return [s.strip() for s in response.choices[0].message.content.split(",") if s.strip()]
