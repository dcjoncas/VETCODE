...
class TempProfilesWorkflowTests(unittest.TestCase):
    ...
    def test_legal_verification_uses_jurisdiction_and_provider_admissions(self):
        ...
        self.assertIn('id="criteriaLicensesLabel" class="criteria-card-title">Credentials', html)
        self.assertIn('id="searchCriteriaTitle">Search criteria', html)
        self.assertIn('id="criteriaSkillsOptions"', html)
        self.assertIn('id="criteriaCitiesOptions"', html)
        self.assertIn('id="criteriaExperienceOptions"', html)
        self.assertIn('id="criteriaLicensesOptions"', html)
        self.assertIn('id="criteriaArrangementOptions"', html)
        self.assertIn('id="criteriaWorkforceOptions"', html)
        self.assertIn('data-criteria-add="skills"', html)
        self.assertIn('data-criteria-add="licenses"', html)
        self.assertIn("MAJOR_US_CANADA_CITIES", html)
        ...
        self.assertIn('formData.append("experience_ranges"', html)
        self.assertIn('formData.append("licenses_or_certifications"', html)
        self.assertIn('const criteriaErrors = refreshSearchCriteriaStatus()', html)
        self.assertIn('if (criteriaErrors.length)', html)
        self.assertIn('or turn off those criteria.', html)

    def test_external_criteria_use_compact_rows_and_each_can_be_switched_off(self):
        html = (PAGES / "mine-candidate-external.html").read_text(encoding="utf-8")

        self.assertNotIn('<section class="criteria-section"', html)
        self.assertEqual(html.count('class="criteria-switch"'), 7)
        self.assertEqual(html.count('role="switch"'), 7)
        self.assertEqual(html.count('name="search-criteria-editor"'), 7)
        self.assertIn("function handleCriteriaEditorToggle", html)
    ...
    def test_shared_workflow_preserves_location_and_hides_secondary_clutter(self):
        ...
        self.assertIn("Determine interest", temp_profiles)
        self.assertIn("Continue to Profile Build", temp_profiles)
