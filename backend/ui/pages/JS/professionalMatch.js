(function (root) {
  ...
  function view(candidate = {}, jobId = "", criteria) {
    ...
    return {
      ...
      label: current && validScore(match.score) ? `${Math.round(match.score)}% requirement support` : current ? "Fit not yet assessable" : jobId ? "Calculate current JD fit" : "Choose a JD to assess fit",
  ...
  function details(match = {}) {
    ...
    return `<div class="match-score-summary"><div class="match-score-number">${validScore(match.score) ? `${Math.round(match.score)}%` : "—"}</div><div><strong>Support for reviewed job requirements</strong><p>${escape(match.reason)}</p><div>${validScore(match.coveragePercent) ? `${Math.round(match.coveragePercent)}% evidence coverage` : "Evidence coverage unavailable"} · not a hiring probability</div></div></div>
      <div class="match-stat-grid">${rows.map((row) => `<section class="match-stat-card"><h4>${escape(row.label)}</h4><strong>${escape(statusLabels[row.status] || 'Unknown')}</strong>${row.required ? ' · required' : ''}${row.scored === false ? ' · outside score' : ''}<p>${escape(row.reason)}</p>${(row.evidence || []).map((item) => `<blockquote>${escape(item.text)}<footer>${escape(item.field)}</footer></blockquote>`).join('')}</section>`).join('')}</div>
  ...
  root.DevReadyProfessionalMatch = { criteriaKey, view, details };
